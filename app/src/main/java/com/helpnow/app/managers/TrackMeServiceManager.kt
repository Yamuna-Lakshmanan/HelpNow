...
import com.helpnow.app.services.TrackMeService
...
class TrackMeServiceManager private constructor(context: Context) {
    ...
    private val smsModule: SmsLocationModule by lazy {
        (contextRef.get()?.applicationContext as? HelpNowAppInterface)?.getSmsLocationModule()
            ?: com.helpnow.app.integration.SmsLocationModuleImpl(contextRef.get()!!)
    }
    ...
    fun onHomeReached() {
        ...
        _welcomeHomeMessage.value = try {
            contextRef.get()?.resources?.getString(com.helpnow.app.R.string.welcome_home)
...
interface HelpNowAppInterface {
    fun getSmsLocationModule(): SmsLocationModule?
}
